...
if ~strcmpi(STYLE,'grid')                     % if not plot grid only
    ...
    if ~isempty(EMARKER2CHANS)
        if strcmpi(STYLE,'blank') & ~plain_blank,
...
if ~strcmpi(STYLE,'blank') % if draw interpolated scalp map
    if ~strcmpi(STYLE,'grid') %  not a rectangular channel grid
        ...
        [Xi,Yi,Zi] = griddata2(inty,intx,intValues,yi',xi,'invdist'); % interpolate data
        [Xi,Yi,ZiC] = griddata2(inty,intx,intContourVals,yi',xi,'invdist'); % interpolate data
...
if ~strcmpi(STYLE,'grid')                     % if not plot grid only
    ...
    if isempty(EMARKERSIZE)
        ...
        elseif length(y)>=64
            EMARKERSIZE = 8; %wad 5
        elseif length(y)>=48
            EMARKERSIZE = 10; %was 6
        elseif length(y)>=32
            EMARKERSIZE = 10; %was 8
        end
    end
    %
    ...
    if strcmp(ELECTRODES,'on')   % plot electrodes as spots
        if isempty(EMARKER2CHANS)
            for i=1:length(y),
                %        hp2 = plot3(y(i),x(i),1*ELECTRODE_HEIGHT,...
                %            EMARKER,'Color',ECOLOR,'markersize',EMARKERSIZE,'linewidth',EMARKERLINEWIDTH, ...
                %            'buttondownfcn',['set(gco,''color'',''r'');']);
                %DG?? I added the below in place of the code above so that you
                %could click on electrodes and see the name
                ...
                hh(i) = text(double(y(i)+0.01),double(x(i)),...
                    ELECTRODE_HEIGHT,'   ','HorizontalAlignment','center',...
                    'VerticalAlignment','middle','Color', ECOLOR,'userdata', labels(i,:) , ...
                    'FontSize',round(EFSIZE*1), 'fontunits','normalized','buttondownfcn', ...
                    ['tmpstr = get(gco, ''userdata'');' ...
                    'set(gco, ''userdata'', get(gco, ''string''));' ...
                    'set(gco, ''string'', tmpstr); clear tmpstr; if strcmpi(get(gco,''backgroundcolor''),''none''),' ...
                    'set(gco,''backgroundcolor'',''w''); else set(gco,''backgroundcolor'',''none''); end;'] );
            end
...
function lowcase=str2lowcase(str)
